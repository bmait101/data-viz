...
pubs <- pubs |>
  filter(!rowname %in% c("26", "29")) |>
  mutate(type = case_when(
    rowname %in% c("4", "15") ~ "Thesis",
    rowname %in% c("1", "18") ~ "Technical Report",
    rowname %in% c("30") ~ "Software",
    journal %in% c("bioRxiv") ~ "Preprint",
    TRUE ~ "Refereed Article"
  )) |>
  # give factor levels for type
  mutate(type = factor(
    type,
    levels = c("Refereed Article", "Preprint", "Technical Report", "Software", "Thesis")
  )) |> 
  left_join(xref_journal, by = "journal") |>
  mutate(Author_lastname = str_split_fixed(author, ",", 2)[, 1]) |>
  mutate(Author_lastname = str_split_fixed(Author_lastname, " ", 2)[, 2]) |>
  mutate(pubid_short = paste0(
    Author_lastname, " et al. ", year, " (", j_short, ")"
  ))
...
ach <-
  ach |>
  left_join(
    pubs |> select(pubid, pubid_short, j_short, type, author), 
    by = c("pubid" = "pubid")
    )
...
ach |>
  ...
  scale_x_continuous(breaks = seq(2013, 2025, 3), limits = c(2012, 2025)) +
